import { Flex, Image, Text, Heading, Button, Box, VStack, HStack, Stack, Badge, useToast } from '@chakra-ui/react';
import { AddIcon, AttachmentIcon, CheckCircleIcon, InfoOutlineIcon, SettingsIcon, StarIcon , } from '@chakra-ui/icons';
...
import { useGetUserDetailQuery, useUpdateUserCartMutation } from '../Redux/service/productsService/userAuth';
import { getUserDataFromCookies } from './GetUserCookiesdata';
const ProductDetail = () => {
    const {id} = useParams()
const userData = getUserDataFromCookies();
const navigate = useNavigate()
    const {data , isError , isLoading} = useGetProductDetailQuery(id)
    const [updateUserCart, {isError:cartError, isLoading:cartLoading}] = useUpdateUserCartMutation()
    const {data:userDetaildata} = useGetUserDetailQuery(userData?.data.user._id)
...
try {
  let res = await  updateUserCart(data)

if(res.data && !res.data?.msg){
  
  document.cookie = `userDataLogin=${JSON.stringify(res?.data)}; path=/`;
  localStorage.setItem("keyupdate", Math.random())
  toast({
    ...
    <Flex p={5} alignItems={"flex-start"} gap={10} flexDirection={{ base: "column", md: "row", lg:"row" }}>
      ...
      <VStack mt={{ base: 0, md: "10rem" }} align="start" spacing={4} maxW={{ base: "100%", md: "50%" }}>
        ...
        <HStack spacing={2} alignItems="center">
          <Text fontWeight="bold" color="black">{"RS " + data?.price2}</Text>
          <Text textDecor="line-through" color="gray.500">{"RS " + data?.price1}</Text>
          <Badge colorScheme="pink">{data?.discount}</Badge>
        </HStack>
        <Text color={"#03A685"}>inclusive of all taxes</Text>
        <Flex alignItems="center" gap={6}>
          <Text>Size</Text>
          <Text color="blue.500" cursor="pointer">Size Chart</Text>
        </Flex>
      ...
      <Box p={4} bg="gray.50" borderRadius="md" boxShadow="sm">
          <HStack spacing={2} alignItems="center" mb={2}>
            ...
            <Heading size="md" color="teal.600">PRODUCT DETAILS</Heading>
          </HStack>
        ...
        <Box p={4} bg="gray.50" borderRadius="md" boxShadow="sm">
          <HStack spacing={2} alignItems="center" mb={2}>
            ...
            <Heading size="md" color="green.600">MATERIAL & CARE</Heading>
          </HStack>
          <VStack align="start" spacing={1}>
        ...
        <Box w="100%" p={4} bg="gray.50" borderRadius="md" boxShadow="sm">
          <HStack spacing={2} alignItems="center" mb={2}>
            ...
            <Heading size="md" color="purple.600">SPECIFICATIONS</Heading>
          </HStack>
          <VStack align="start" spacing={1}>
        ...
        <Box w="100%" p={4} bg="gray.50" borderRadius="md" boxShadow="sm">
          <HStack spacing={2} alignItems="center" mb={2}>
            ...
            <Heading size="md" color="yellow.600">RATINGS & REVIEWS</Heading>
          </HStack>
          <HStack spacing={1}>
